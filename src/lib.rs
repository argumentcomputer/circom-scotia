    ...
    G2: Group<Base = <G1 as Group>::Scalar>,
{
    ...
    for i in 0..iteration_count {
        ...
        let witness = if is_wasm {
            generate_witness_from_wasm::<F<G1>>(&witness_generator_file, &input_json, Path::new(""))
                .await
        } else {
            let witness_generator_file = match &witness_generator_file {
                FileLocation::PathBuf(path) => path,
            ...
            generate_witness_from_bin::<F<G2>>(&witness_generator_file, &input_json, Path::new(""))
        };
        let circuit = CircomCircuit {
            r1cs: r1cs.clone(),
            witness: Some(witness),
    ...
    let mut recursive_snark: Option<RecursiveSNARK<G1, G2, C1<G1>, C2<G2>>> = None;
