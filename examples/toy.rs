...
fn run_test<G1, G2>(circuit_filepath: String, witness_gen_filepath: String)
where
...
fn main() {
    type Bn254 = provider::bn256_grumpkin::bn256::Point;
    type Grumpkin = provider::bn256_grumpkin::grumpkin::Point;

    type Pallas = pasta_curves::pallas::Point;
    type Vesta = pasta_curves::vesta::Point;

    for group_name in ["bn254"] {
        ...
        for witness_gen_filepath in [
            ...
            match group_name {
                "bn254" => {
                    run_test::<Bn254, Grumpkin>(circuit_filepath.clone(), witness_gen_filepath)
                }
                "pasta" => {
                    run_test::<Pallas, Vesta>(circuit_filepath.clone(), witness_gen_filepath)
                }
                _ => panic!("unreachable"),
            }
        }
